...
namespace FF1Lib
{
	public class Flags : IIncentiveFlags, IMapEditFlags, IScaleFlags, IFloorShuffleFlags
	{
		...
		public static string EncodeFlagsText(Flags flags)
		{
			...
			var properties = typeof(Flags).GetProperties(BindingFlags.Public | BindingFlags.Instance);
			var flagproperties = properties.Where(p => p.CanWrite).OrderBy(p => p.Name);

			foreach (var fp in flagproperties)
			{
				if (fp.PropertyType == typeof(bool))
				{
					sum = AddBoolean(sum, (bool)fp.GetValue(flags));
				}
				else if (Nullable.GetUnderlyingType(fp.PropertyType) == typeof(bool))
				{
					sum = AddTriState(sum, (bool?)fp.GetValue(flags));
				}
				else if (fp.PropertyType.IsEnum)
				{
					sum = AddNumeric(sum, Enum.GetValues(fp.PropertyType).Cast<int>().Max() + 1, (int)fp.GetValue(flags));
				}
				else if (fp.PropertyType == typeof(int))
				{
					var ia = fp.GetCustomAttribute<IntegerFlagAttribute>();

					if (ia != null)
					{
						var val = (int)fp.GetValue(flags);
						sum = AddNumeric(sum, ia.Max - ia.Min + 1, val - ia.Min);
					}
				}
				else if (fp.PropertyType == typeof(double))
				{
					var ia = fp.GetCustomAttribute<DoubleFlagAttribute>();

					if (ia != null)
					{
						var val = (double)fp.GetValue(flags);

						var radix = (int)Math.Ceiling((ia.Max - ia.Min) / ia.SmallestStep) + 1;
						var enc = (int)Math.Round((val - ia.Min) / ia.SmallestStep, 0);

						sum = AddNumeric(sum, radix, enc);
					}
				}
			}
		...
		public static Flags DecodeFlagsText(string text)
		{
			...
			var properties = typeof(Flags).GetProperties(BindingFlags.Public | BindingFlags.Instance);
			var flagproperties = properties.Where(p => p.CanWrite).OrderBy(p => p.Name).Reverse();
			...
			foreach (var fp in flagproperties)
			{
				if (fp.PropertyType == typeof(bool))
				{
					fp.SetValue(flags, GetBoolean(ref sum));
				}
				else if (Nullable.GetUnderlyingType(fp.PropertyType) == typeof(bool))
				{
					fp.SetValue(flags, GetTriState(ref sum));
				}
				else if (fp.PropertyType.IsEnum)
				{
					fp.SetValue(flags, GetNumeric(ref sum, Enum.GetValues(fp.PropertyType).Cast<int>().Max() + 1));
				}
				else if (fp.PropertyType == typeof(int))
				{
					var ia = fp.GetCustomAttribute<IntegerFlagAttribute>();
					...
					if (ia != null)
					{
						var val = GetNumeric(ref sum, ia.Max - ia.Min + 1);
						fp.SetValue(flags, val + ia.Min);
					}
				}
				else if (fp.PropertyType == typeof(double))
				{
					...
					if (ia != null)
					{
						...
						var val = GetNumeric(ref sum, radix);
						var dec = Math.Min(Math.Max(((double)val * ia.SmallestStep) + ia.Min, ia.Min), ia.Max);

						fp.SetValue(flags, dec);
					}
				}
			}
